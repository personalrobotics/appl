 ...
 * complexity is O(no. of particles * search depth * |S|), time complexity for
 * pre-processing is O(no. of particles * search depth * |S| * |A|) and time
 * complexity for queries is O(no. of particles).
 *
 * The upper bound for a set of particles is the average upper bounds of its
 * particles. The upper bound for a particle is computed as the best possible
 * sequence of actions that can lead to a terminal state using the random 
 * number sequence for that particle.
 */
template<typename T>
    ...
    for (int j = 0; j <= Globals::config.search_depth; j++)
      for (int k = 0; k < model.NumStates(); k++)
        if (upper_bound_memo_[i][j][k] == -Globals::INF) {
          Particle<T>* p = model.Allocate();
          p->state = k;
          p->id = i;
          p->wt = 0; // dummy
          UpperBound(*p, j, model);
          model.Free(p);
        }
  }
}
