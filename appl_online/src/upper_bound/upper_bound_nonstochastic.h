...
class UpperBoundNonStochastic : public IUpperBound<T> {
 ...
 private:
  // We use dynamic programming to compute the best upper bound action and
  // value for a state, with horizon = max depth of the tree. We use this value
  // regardless of the depth at which the upper bound is requested, as it 
  // is always a tighter bound. We couldn't do this in the non-stochastic case
  // because the upper bound depended on the sequence of random numbers.
...
UpperBoundNonStochastic<T>::UpperBoundNonStochastic(
  ...
  // Maintain pointers to 2 levels of the array and swap them each iteration
  // to save space.
