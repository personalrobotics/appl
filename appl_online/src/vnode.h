...
class VNode {
 public:
  // Note that the ctor move-constructs the particles instead of copying them,
  // for efficiency.
  VNode(vector<Particle<T>*>&& particles, double l, double u, int depth,
      ...
      : particles_(particles),
        ...
        in_tree_(it),
        n_tree_nodes_(it) {
    Globals::ValidateBounds(lbound_, ubound_);
    node_count_++;
  }
  ...
  vector<QNode<T>>& qnodes() { return qnodes_; }

  static void set_model(const Model<T>& model) { model_ = &model; }

  // Returns the action that achieves the node's lower bound. Since at the end
  // of the search we need the optimal action only at the root of the search 
  // tree, we avoid storing it as a property at every node, instead computing
  // it in this method when it's needed. Prerequisite: The node must have children.
  int LowerBoundAction() const;
 ...
 private:
  ...
  double ubound_;
  const int depth_;
  const double default_value_; // Value of the default policy (= lbound value
                               // before any backups are performed)
  int pruned_action_;    // Best action at the node after pruning
  const double weight_;  // Sum of particle weights at this belief
...
template<typename T>
int VNode<T>::LowerBoundAction() const {
  int a_star = -1;
